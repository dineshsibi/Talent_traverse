...
try {
  ...
  if (isset($_GET['ajax'])) {
    ...
    if ($clientName) {
      ...
      $query = $pdo->prepare("SELECT DISTINCT state FROM consolidated_data WHERE client_name = ? ORDER BY state ASC");
      ...
      if (!empty($selectedStates)) {
        ...
        $query = $pdo->prepare("SELECT DISTINCT location_code FROM consolidated_data WHERE client_name = ? AND state IN ($statePlaceholders) ORDER BY location_code ASC");
  ...
  $clientList = $pdo->query("SELECT DISTINCT client_name FROM consolidated_data ORDER BY client_name ASC")->fetchAll(PDO::FETCH_COLUMN);
...
<body>
  ...
  <nav class="navbar">
    ...
    <div class="nav-links">
       <a href="upload.php?category=<?= htmlspecialchars($_GET['category'] ?? 'factory') ?>">Back</a>
