    ...
    <table>
        <tr>
            <td colspan="16" class="title">
                FORM-IV <br>
                The Sikkim Minimum Wages Rules, 2005, [See rule 25 (2)] <br>
                Overtime Register for Workers 
            </td>
        </tr>
        <tr>
            <td colspan="6" class="label" style="text-align: left;">Name and Address of the Establishment / Factory</td>
            <td colspan="10" style="text-align: left;"><?= htmlspecialchars($client_name . ' , ' . ($first_row['branch_address'] ?? '')) ?></td>
        </tr>
        <tr>
            <td colspan="6" class="label" style="text-align: left;">Month & Year</td>
        ...
        <tr>
            ...
            <th style="width: 8%">Total overtime worked or production  in case of piece workers</th>
            ...
            <th style="width: 7%">Date on which over time payments made</th>
        </tr>
        <tr>
            <th>1</th>
            <th>2</th>
        ...
        <?php if (!empty($stateData)): ?>
            <?php $i = 1; foreach ($stateData as $row): ?>
                ...
                <tr>
                    ...
                    <td><?= $normalHours ?></td>
                    <td><?= number_format($normalRate, 2) ?></td>
                    <td><?= htmlspecialchars (round ($calculation1 ?? '')) ?></td>
                    <td><?= number_format($normalEarnings, 2) ?></td>
                    <td><?= number_format($overtimeEarnings, 2) ?></td>
        ...
        <?php else: ?>
            <tr>
                <td colspan="16" style="text-align:center;">No data found for Sikkim</td>
            </tr>
        <?php endif; ?>
    </table>
</body>
